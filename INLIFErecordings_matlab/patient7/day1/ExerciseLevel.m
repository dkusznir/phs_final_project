...
if p(length(p))<=x(length(x))
    temp_2 = X(max(find(x<=p(length(p)))));
else
    temp_2 = X(length(x));
end
...
text(5,6.5,'Day','FontSize',20);
temp = fix(6.5*length(X)/9.5);
a = [temp,temp,length(X),length(X)];
b = [6,7,7,6];
c = fill(a,b,rgb('MidnightBlue'));
set(c,'EdgeColor','None');
text(temp+5,6.5,'Night','FontSize',20,'Color','white');
%%-------------------------------------------------------------------------

%%-------------------------------------------------------------------------
%% x-Axis
xlim([1,length(x)]);
name = {'11:30AM','1:00PM','2:30M','4:00PM','5:30PM','7:00PM','8:30PM'};
step = fix(length(X)/6.33);
kk = 1:7;
value = [1,kk*step];
...
title('Patient7 10/23/2016-10/24/2016 Record','FontSize',15);
%%-------------------------------------------------------------------------
